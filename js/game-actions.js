...
function stageActivateEffect(cardNum) {
  ...
  if (act.defausse) {
    ...
    showDefausseModal(playIndex, act, candidates);
    return true;
  }
...
function showDefausseModal(playIndex, act, candidates) {
  const cardInstance = gameState.play[playIndex];
  const fd = getFaceData(cardInstance);
  ...
  $('#defausseChoiceTitle').text(`🤝 ${fd.nom} — Choisir une carte à défausser`);
  ...
  candidates.forEach((ci) => {
    ...
    const resInfo = f.ressources && f.ressources.length
      ...
      : '—';
    html += `<button onclick="confirmDefausse(${playIndex}, ${actualIdx})" class="sacrifice-choice-btn">
  ...
  html += `<button onclick="cancelDefausseModal()" class="btn btn-sm"
  ...
  window._pendingDefaussePlayIndex = playIndex;
  window._pendingDefausseAct = act;
  $('#defausseChoiceBody').html(html);
  new bootstrap.Modal(document.getElementById('defausseChoiceModal')).show();
}
...
function cancelDefausseModal() {
  if (document.activeElement) document.activeElement.blur();
  bootstrap.Modal.getInstance(document.getElementById('defausseChoiceModal'))?.hide();
  const playIndex = window._pendingDefaussePlayIndex;
  window._pendingDefaussePlayIndex = null;
  window._pendingDefausseAct = null;
...
function confirmDefausse(plainesPlayIndex, defaussePlayIndex) {
  if (document.activeElement) document.activeElement.blur();
  bootstrap.Modal.getInstance(document.getElementById('defausseChoiceModal'))?.hide();

  const act = window._pendingDefausseAct;
  window._pendingDefaussePlayIndex = null;
  window._pendingDefausseAct = null;
  ...
  const defausseCard = gameState.play[defaussePlayIndex];
  const defausseName = getFaceData(defausseCard).nom;

  const idxs = [plainesPlayIndex, defaussePlayIndex].sort((a, b) => b - a);
  ...
  gameState.staging.push({
    ...
    newFace: null,
    cout: act.cout || [],
    defausseCardInstance: defausseCard,
    fromRetainedCards: fromRetained
  });

  addLog(`🟢 <span class="log-card">${activatingCardName}</span> + <span class="log-card">${defausseName}</span> — en attente de confirmation (${resStr}).`);
  updateUI();
}
...
function confirmSacrifice(plainesPlayIndex, sacrificePlayIndex) {
  ...
  gameState.staging.push({
    ...
    newFace: null,
    cout: act.cout || [],
    sacrificeCardInstance: sacrificeCard,
    fromRetainedCards: fromRetained
  });
